...
extern void mpt_dassl_fini(MPT_SOLVER_STRUCT(dassl) *data)
{
	...
	if (data->dmas) { free(data->dmas); data->dmas = 0; }
	
	mpt_vecpar_cktol(&data->rtol, 0, 0, __MPT_IVP_RTOL);
	mpt_vecpar_cktol(&data->atol, 0, 0, __MPT_IVP_ATOL);
}

extern int mpt_dassl_init(MPT_SOLVER_STRUCT(dassl) *data)
{
	/* allocate inital space for parameters */
	data->rwork.iov_base = data->iwork.iov_base = 0;
	if (!mpt_vecpar_alloc(&data->rwork, 48, sizeof(double))) {
		return -1;
	}
	if (!mpt_vecpar_alloc(&data->iwork, 24, sizeof(int))) {

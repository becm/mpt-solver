...
static int setJacobian(MPT_SOLVER_STRUCT(dassl) *data, MPT_INTERFACE(source) *src)
{
	...
	if (src->_vptr->conv(src, 'k', &key) < 0) return -2;
	
	switch (key[0]) {
		case 'F': data->info[4] = 1;
		case 'f': data->info[5] = 0; return 0;
		case 'B': data->info[4] = 1;
		case 'b': data->info[5] = 1; break;
...
static int setInfo3(MPT_SOLVER_STRUCT(dassl) *data, MPT_INTERFACE(source) *src)
{
	...
	if ((len = src->_vptr->conv(src, 'i', data->info+2)) < 0) return -2;
...
static int setTStop(MPT_SOLVER_STRUCT(dassl) *data, MPT_INTERFACE(source) *src)
{
	...
	if ((len = src->_vptr->conv(src, 'd', data->rwork.iov_base)) < 0) return -2;
	data->info[3] = len ? 1 : 0;
	return len;
}
static int setHMax(MPT_SOLVER_STRUCT(dassl) *data, MPT_INTERFACE(source) *src)
{
	...
	if ((len = src->_vptr->conv(src, 'd', ((double *) data->rwork.iov_base) + 1)) < 0) return -2;
	data->info[3] = len ? 1 : 0;
	return len;
}
static int setH0(MPT_SOLVER_STRUCT(dassl) *data, MPT_INTERFACE(source) *src)
{
	...
	if ((len = src->_vptr->conv(src, 'd', ((double *) data->rwork.iov_base) + 2)) <= 0) return -2;
	data->info[7] = len ? 1 : 0;
	return len;
}
static int setMaxOrd(MPT_SOLVER_STRUCT(dassl) *data, MPT_INTERFACE(source) *src)
{
	...
	if ((len = src->_vptr->conv(src, 'i', data->info+8)) < 0) return -2;
...
static int setInfo10(MPT_SOLVER_STRUCT(dassl) *data, MPT_INTERFACE(source) *src)
{
	...
	if ((len = src->_vptr->conv(src, 'i', data->info+9)) < 0) return -2;
...
static int setInfo11(MPT_SOLVER_STRUCT(dassl) *data, MPT_INTERFACE(source) *src)
{
	...
	if ((len = src->_vptr->conv(src, 'i', data->info+10)) < 0) return -2;

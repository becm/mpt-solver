...
extern int sundials_vector_set(N_Vector *v, long len, MPT_INTERFACE(metatype) *src)
{
	...
	if (src->_vptr->conv(src, MPT_value_toArray(MPT_SOLVER_ENUM(SundialsRealtype)) | MPT_ENUM(ValueConsume), &arr) >= 0) {
	...
	if (src->_vptr->conv(src, MPT_value_toVector(MPT_SOLVER_ENUM(SundialsRealtype)) | MPT_ENUM(ValueConsume), &vec) >= 0) {
		if (vec.iov_len != size) {
			return MPT_ERROR(BadArgument);
		}
		if (!(next = sundials_nvector_new(len))) {
	...
	if (src->_vptr->conv(src, MPT_SOLVER_ENUM(SundialsRealtype) | MPT_ENUM(ValueConsume), &val) < 0) {
